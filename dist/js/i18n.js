 ...
 * Prioridade:
 *   1. Dados embutidos inline em <script id="i18n-data"> (gerado por scripts/build.mjs
 *      no build de produção — não depende de rede nem de fetch()).
 *   2. fetch('locales/<lang>.json') — usado em desenvolvimento, servindo o projeto-fonte.
 */
let inlineDataCache;

function getInlineData() {
  if (inlineDataCache !== undefined) return inlineDataCache;
  const el = document.getElementById('i18n-data');
  if (!el) {
    inlineDataCache = null;
    return inlineDataCache;
  }
  try {
    inlineDataCache = JSON.parse(el.textContent);
  } catch (err) {
    console.warn('[i18n] dados inline inválidos, caindo para fetch():', err);
    inlineDataCache = null;
  }
  return inlineDataCache;
}
...
async function loadTranslations(lang) {
  ...
  if (inline && inline[lang]) return inline[lang];
